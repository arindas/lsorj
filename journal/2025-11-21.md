...
- At a high level, when setting a new state we need to ensure some pre-condition
     - This pre-condition includes checking whether the current state equals some specific state
       before the transition
     - For this, we access the state attribute 2 times:
          - 1 read to check the current state
          - 1 write to write the new state
     - We neeed to ensure that this is serialized, we enforce this with memory barrier
- Sometimes before transitioning to the new state, we need to set an intermedidate state
     - This results in 3 accesses to the state attribute:
          - 1 write of the intermiedate state
          - 1 read of the intermedidate state
          - 1 write after checking that the intermedidate state has been written
     - For example, when waking up a task, we first set it's state to `TASK_UNINTERRUPTIBLE`.
       Once the scheduler reads this write, it attempts to write the state with `TASK_RUNNING`
- If waking up a task fails, we put it back on the runqueue
- The runqueue contains a red black tree of tasks ordered by their priority
